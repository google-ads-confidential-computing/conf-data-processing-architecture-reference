...
public class GetActiveEncryptionKeysCache extends KeyDbCache<String, ImmutableList<EncryptionKey>> {
  ...
  public GetActiveEncryptionKeysCache(
    ...
    this(keyDb, enableCache, cacheRefresh, MINUTES, logMetricHelper);
  }

  @VisibleForTesting
  GetActiveEncryptionKeysCache(
      KeyDb keyDb,
      Boolean enableCache,
      ...
      LogMetricHelper logMetricHelper) {
    super(
        CacheBuilder.newBuilder().refreshAfterWrite(cacheRefresh, timeUnit).maximumSize(100),
        enableCache,
        "GetActiveEncryptionKeysCache",
        logMetricHelper);
    this.keyDb = keyDb;
  ...
  ImmutableList<EncryptionKey> readDb(String setName) throws ServiceException {
    return keyDb.getActiveKeys(setName, 0);
  }
}
