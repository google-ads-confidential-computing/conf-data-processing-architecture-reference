...
public abstract class GcpCreateSplitKeyTaskTestBase extends CreateSplitKeyTaskBaseTest {
  ...
  private void createKeysWithoutExpirationAndTtl(String setName) throws Exception {
    ...
    assertThat(keyDb.getActiveKeys(setName, 1, now().plus(Duration.ofHours(1))).size())
        .isEqualTo(1);
    assertThat(keyDb.getActiveKeys(setName, 1, now().plus(Duration.ofDays(1))).size()).isEqualTo(1);
    assertThat(keyDb.getActiveKeys(setName, 1, now().plus(Duration.ofDays(365))).size())
        .isEqualTo(1);
    assertThat(keyDb.getActiveKeys(setName, 1, now().plus(Duration.ofDays(36500))).size())
        .isEqualTo(1);
  }
