...
variables {
  ...
  request_latencies_alarm_config = {
    duration_sec    = 0
    enable_alarm    = false
    eval_period_sec = 0
    threshold_ms    = 0
...
run "enables_alarms" {
  ...
  assert {
    condition     = length(google_monitoring_alert_policy.error_5xx_count) == 1
    error_message = "Didn't enable alarms"
  }
  assert {
    condition     = length(google_monitoring_alert_policy.non_5xx_error_count) == 1
    error_message = "Didn't enable alarms"
  }
  assert {
    condition     = length(google_monitoring_alert_policy.request_latencies) == 1
    error_message = "Didn't enable alarms"
  }
}
